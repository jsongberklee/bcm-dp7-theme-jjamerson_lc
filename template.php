...
function jjamerson_lc_preprocess_region(&$region) {
  if ( isset($node['language']) ) {
    $lang = $node['language'];
  } else {
    $lang = 'und';
...
function jjamerson_lc_process_html(&$variables) {
...
function jjamerson_lc_process_views_view_field(&$variables) {
...
function jjamerson_lc_process_field(&$field) {
  global $user;
...
function jjamerson_lc_preprocess_node(&$node) {
  global $user; // Access to Drupal's global for currently logged-in user
 ...
 */
function jjamerson_lc_css_alter(&$css) {
 ...
 */
function jjamerson_lc_js_alter(&$js) {
  //unset($js[ path_to_theme() . '/js/jjamerson_lc.min.js' ])
  if ( isset($_GET['render-only']) || isset($_GET['content-only'])) {
    unset($js[ 'sites/all/modules/patched/admin_menu/admin_menu.js' ]);
  }
}

function jjamerson_lc_menu_link($variables) {
